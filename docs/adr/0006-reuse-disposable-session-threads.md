...
Cold reconstruction injects the logged prefix into a new thread. The final ordinary user message is supplied through `turn/start.input`; later ordinary user messages use the same native append path. `thread/inject_items` remains a reconstruction mechanism rather than the normal turn transport because a real two-turn OAuth probe demonstrated cache reuse through native turn input while an injected second turn did not produce a new cache hit.

When Codex requests a declared Harness dynamic tool, the adapter emits the normal Harness `tool-call` and leaves the App Server JSON-RPC request pending. The next request may resume that Codex turn only when its derived history is the expected prefix followed by the matching `function_call_output`. The lease replies with that logged result, and Codex continues the same turn. Any additional or mismatched history disposes the lease and reconstructs a new thread instead of guessing how to merge concurrent input.

Successful output advances the lease's expected history from the same replay items written to the Harness assistant message. Cancellation, transport failure, malformed output, consumer abandonment, or an unrecognized state transition disposes the lease. Native Codex compaction may finish the active Codex turn, but it makes the lease ineligible for another Harness request: opaque native compaction state is provider trajectory, not a portable Harness checkpoint. Harness compaction remains the durable history-replacement mechanism and naturally forces a cold thread from its logged summary.
...
- Inject every newly logged item before an empty turn. This preserves the existing stateless encoder but did not demonstrate cross-turn cache reuse. Native `turn/start.input` is used only for the exact trailing user-message case; every less precise continuation falls back to cold reconstruction.
