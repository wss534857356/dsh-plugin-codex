...
Advertise image input only for a configured model whose current catalog snapshot or explicit deployment configuration affirms it. The shipped App Server `0.153.3` snapshot records image input for GPT-6 Astra, GPT-5.6 Sol/Terra/Luna, GPT-5.5, GPT-5.4, and GPT-5.4 Mini, and text-only input for GPT-5.3 Codex Spark. Custom entries without `inputModalities` and uncatalogued model ids remain text-only. Re-probe `model/list` whenever the pinned Codex version changes because the account/server catalog is not pinned by the npm package.
