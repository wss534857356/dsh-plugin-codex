...
- App Server starts with `--strict-config`; the selected `CODEX_HOME` must contain only keys accepted by Codex `0.154.0`. A stale key such as the removed `network_access` setting makes the process exit before `initialize`; update that native config or point `env.CODEX_HOME` at a compatible logged-in home.
